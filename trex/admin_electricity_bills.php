    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'delete':
                ...
                if (!empty($bill_id)) {
                    try {
                        $stmt = $pdo->prepare("DELETE FROM bills WHERE id = ? AND service_type = 'electricity'");
...
try {
    $stmt = $pdo->query("SELECT id, name, last_name, electric_counter_number FROM users ORDER BY name, last_name");
...
try {
    $stmt = $pdo->query("
        SELECT b.*, u.name, u.last_name, u.electric_counter_number 
        FROM bills b 
        JOIN users u ON b.user_id = u.id 
        WHERE b.service_type = 'electricity' 
        ORDER BY b.created_at DESC
    ");
    $electricity_bills = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $electricity_bills = [];
}
?>
...
<head>
    ...
    <title>GazTronik - Gestion factures électricité</title>
...
<body>
    <div class="admin-header">
        <div class="admin-title">
            ...
            <h1>Gestion factures électricité</h1>
        </div>
    ...
    <div class="content-grid">
            ...
            <form method="POST" id="billForm">
                    ...
                    <select id="user_id" name="user_id" required>
                        ...
                        <?php foreach ($users as $user): ?>
                            <option value="<?php echo $user['id']; ?>">
                                <?php echo htmlspecialchars($user['name'] . ' ' . $user['last_name'] . ' - ' . $user['electric_counter_number']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="bill_number" name="bill_number" required 
                           placeholder="Ex: ELEC-2024-001" 
                           value="ELEC-<?php echo date('Y'); ?>-<?php echo str_pad(count($electricity_bills) + 1, 3, '0', STR_PAD_LEFT); ?>">
                </div>
        ...
        <div class="bills-list-section">
            <h2 class="section-title">
                ...
                Factures électricité existantes
            </h2>

            <?php if (!empty($electricity_bills)): ?>
                        ...
                        <div class="bill-header">
                            <div>
                                ...
                                <div class="bill-user">
                                    ...
                                    - Compteur: <?php echo htmlspecialchars($bill['electric_counter_number']); ?>
                                </div>
                            </div>
