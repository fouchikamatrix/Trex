<?php
$page_title = "Actualités électricité";
session_start();
...
// Get electricity news
try {
    $stmt = $pdo->prepare("
        SELECT * FROM news 
        WHERE service_type IN ('electricity', 'both') AND status = 'published'
    ...
    $news = [];
    error_log("Electricity news error: " . $e->getMessage());
}
...
<div class="news-container">
    <div class="news-header">
        ...
        <h1>Actualités du service électrique</h1>
        <p>Restez informé des dernières annonces et mises à jour du service électrique</p>
    </div>
...
if (!empty($news)) {
    foreach ($news as $article) {
        ...
        switch($article['category']) {
            ...
            case 'outage':
                ...
                break;
            case 'update':
                $categoryText = 'Mise à jour';
                break;
            case 'announcement':
                $categoryText = 'Annonce';
                break;
            case 'emergency':
                $categoryText = 'Urgence';
                break;
            default:
                $categoryText = ucfirst($article['category']);
        }
    ...
    <div class="no-news">
        ...
        <p>Il n\'y a actuellement aucune actualité ou annonce du service électrique. Revenez plus tard pour les mises à jour.</p>
    </div>';
}
