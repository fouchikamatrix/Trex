...
// Get pending gas bills
try {
    $stmt = $pdo->prepare("
        SELECT * FROM bills 
        WHERE user_id = ? AND service_type = 'gas' AND status = 'pending'
    ...
    $pending_bills = [];
    error_log("Erreur factures paiement gaz: " . $e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($bill_id) || empty($payment_method) || empty($amount)) {
        ...
        try {
            ...
            $error_message = 'Échec du traitement du paiement. Veuillez réessayer.';
...
<div class="payment-container">
    <div class="payment-header">
        ...
        <h1>Paiement gaz</h1>
        <p>Payez vos factures de gaz en ligne en toute sécurité</p>
    </div>
...
if (!empty($pending_bills)) {
    ...
    $content .= '
        <div class="no-bills">
            ...
            <p>Vous n\'avez aucune facture de gaz en attente de paiement pour le moment.</p>
            <a href="gas_bill.php" style="color: #ff6b35; text-decoration: none; font-weight: 600; margin-top: 15px; display: inline-block;">
...
function checkFormCompletion() {
    ...
    if (billSelected && paymentMethodSelected) {
        ...
        submitBtn.innerHTML = \'<i class="fas fa-lock"></i> Sélectionner facture et méthode\';
    }
}
