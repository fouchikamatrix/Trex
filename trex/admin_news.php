        ...
        switch ($_POST['action']) {
            case 'create':
                ...
                if (empty($title) || empty($content) || empty($category) || empty($service_type)) {
                    ...
                    try {
                        $stmt = $pdo->prepare("
                            INSERT INTO news (title, content, category, service_type, priority, author, status, expires_at, published_at, created_at) 
                            VALUES (?, ?, ?, ?, ?, ?, 'published', ?, NOW(), NOW())
                        ");
                        $stmt->execute([$title, $content, $category, $service_type, $priority, $author, $expires_at]);
                        $success_message = 'Actualité créée avec succès !';
                    } catch (PDOException $e) {
                        $error_message = 'Erreur lors de la création de l\'actualité.';
                    }
                }
                break;
...
<head>
    ...
    <title>VoltGaz - Gestion des actualités</title>
        ...
        <div class="form-section">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" required placeholder="Titre de l'actualité">
                </div>
